...
back11 = now - timedelta(hours=11)
b11 = back11.strftime("%Y%m%d%H")
print b11
...
if os.path.isfile(input_dir+b11+'00/gfsblock.png') :
...
#  os.utime(lockfiles_dir+'mv_hrrr_test.lock', None)
    mkdir_cmd  = 'ssh ratchet mkdir -m 777 -p '+output_dir+b11+'/full'
    print mkdir_cmd 
    os.system(mkdir_cmd)
    sync_cmd = 'rsync -avz '+input_dir+b11+'00/gfsblock.png ratchet:' + output_dir+b11+'/full/block_f00.png --delete --rsh=ssh'
    print sync_cmd
    os.system(sync_cmd)
    makeimageslist_cmd = 'ssh ratchet.fsl.noaa.gov \'(cd ' + output_dir+b11+'/full; /bin/ls -1 > images.txt; chmod 777 images.txt)\''
    print makeimageslist_cmd
    os.system(makeimageslist_cmd)
    chm_cmd  = 'ssh ratchet.fsl.noaa.gov chmod -R a=rwx '+output_dir+b11
    print chm_cmd
    os.system(chm_cmd)
