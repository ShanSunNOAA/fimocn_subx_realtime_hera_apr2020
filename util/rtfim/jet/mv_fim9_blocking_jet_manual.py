...
print str(now)
input_dir = '/pan2/projects/nrtrr/amb-verif/blocking/fim9/'
output_dir = '/data/amb/graphics/fim9_blocking_jet/'
lockfiles_dir = '/home/rtfim/lockfiles/'

for bdate in bdates :
  if os.path.isfile(input_dir+bdate+'00/fim9block.png') :
    if not os.path.isfile(lockfiles_dir+'mv_fim9_blocking_jet.lock') :
      open(lockfiles_dir+'mv_fim9_blocking_jet.lock', 'w')
#  os.utime(lockfiles_dir+'mv_hrrr_test.lock', None)
      mkdir_cmd  = 'ssh ratchet mkdir -m 777 -p '+output_dir+bdate+'/full'
      print mkdir_cmd 
      os.system(mkdir_cmd)
      sync_cmd = 'rsync -avz '+input_dir+bdate+'00/fim9block.png ratchet:' + output_dir+bdate+'/full/block_f00.png --delete --rsh=ssh'
      print sync_cmd
      os.system(sync_cmd)
      ...
      os.remove(lockfiles_dir+'mv_fim9_blocking_jet.lock')
