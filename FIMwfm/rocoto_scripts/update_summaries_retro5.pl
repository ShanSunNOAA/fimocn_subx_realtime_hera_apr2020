sub update_summaries_retro5 {
    ...
    my $query = "";
    my $sth;
    my $table = "surface_sums.${model}_${fcst_len}_metar_$region";
    my $madis_table = "$db_name.${model}p";
    if($fcst_len == 1) {
	$madis_table .= "1f";
    }
...
from $db_name.$obs_table as o
     ,$db_name.${model}p as m #ignore index (time_id)
     ,$db_name.metars as s
where 1=1
