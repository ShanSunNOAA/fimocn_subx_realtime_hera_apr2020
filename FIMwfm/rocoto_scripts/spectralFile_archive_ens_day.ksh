...
##
## spectralFile_archive_ens_day.ksh
...
gefs_spectral_dir="/BMC/fim/1year/FIMENS_RETRO_gf/${YYYY}/"       ## Dec 2013
echo $gefs_spectral_dir
echo ${yyyymmdd}.tar

# Set up a work directory and cd into it
workdir=${FIM_RUN}/${yyyymmdd}${HOUR}
echo ${workdir}
if [[ -d ${workdir} ]]; then
  ${RM} -rf ${workdir}
fi
...
htar -xvf ${gefs_spectral_dir}/${yyyymmdd}00.tar ensics_\*
status=$?
if [ $status != 0 ]; then
