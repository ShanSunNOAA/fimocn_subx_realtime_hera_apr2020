...
fhr = sys.argv[2]
gribPath = sys.argv[3]
gribPathGfsEnkf = sys.argv[4]
fimensPath = sys.argv[5]
tcvitalsPath = sys.argv[6]
nmembers = sys.argv[7]
nmembers=int(nmembers)
nanals = nmembers * 2
print 'in plotens_FIM_GFS.py: date: ' , date
print 'in plotens_FIM_GFS.py: gribPath: ' , gribPath
print 'in plotens_FIM_GFS.py: gribPathGfsEnkf: ' , gribPathGfsEnkf
print 'in plotens_FIM_GFS.py: fimensPath: ' , fimensPath
print 'in plotens_FIM_GFS.py: tcvitalsPath: ' , tcvitalsPath
    ...
    if lat_0 is not None:
        ...
        for nanal in range(1,nanals+1):
            ...
            if nanal <= nmembers:
                ...
                datapath = gribPathGfsEnkf
                filename = os.path.join(datapath,"%s%03i%s000%03i_mem%02i"%(date[2:4],julday,hr,int(fhr),nanal))
            print 'datapath: ',datapath
            ...
            if nanal <= nanals:
                ...
                plt.title('lo res cntl (t382)',fontsize=10)
            elif nanal == nanals+3:
                plt.title('hi res cntl (t878)',fontsize=10)
            elif nanal == nanals+2:
                plt.title('opnl gfs (t574)',fontsize=10)
            elif nanal == nanals+4:
                plt.title('verifying anal (enkf mean)',fontsize=10)
            # mslp plot
            if isMSL:
              plt.figure(2)
              ax2 = plt.subplot(5,4,nanal)
              ...
              if nanal <= nanals:
                  ...
                  plt.title('lo res (t382) cntl',fontsize=10)
              elif nanal == nanals+3:
                  plt.title('hi res (t878) cntl',fontsize=10)
              elif nanal == nanals+2:
                  plt.title('opnl gfs (t574)',fontsize=10)
              elif nanal == nanals+4:
                  plt.title('verifying anal (enkf mean)',fontsize=10)
            ...
            if fhr > '000' and isPrecip:
               ...
               if nanal <= nanals:
                   ...
                   plt.title('verifying anal (enkf mean)',fontsize=10)
            grbs.close()
        fig1.text(0.5,0.96,'FIM-40km (1-10) GEFS (11-20) 10-m wind ens %s-hr fcst for %s from %s' %(fhr,strmid,date),
                horizontalalignment='center',fontsize=16)
        fig1.text(0.5,0.075,'NOAA/ESRL Global Systems Division',
                horizontalalignment='center',fontsize=12)
        fig2.text(0.5,0.96,'FIM-40km (1-10) GEFS (11-20) MSLP ens %s-hr fcst for %s from %s' %(fhr,strmid,date),
                horizontalalignment='center',fontsize=16)
        fig2.text(0.5,0.075,'NOAA/ESRL Global Systems Division',
                horizontalalignment='center',fontsize=12)
        fig4.text(0.5,0.96,'FIM-40km (1-10) GEFS (11-20) PWAT ens %s-hr fcst for %s from %s' %(fhr,strmid,date),
                horizontalalignment='center',fontsize=16)
        ...
        if fhr > '000' and isPrecip:
            ...
            fig3.text(0.5,0.96,'FIM-40km (1-10) GEFS (11-20) Precip (mm) ens %s-%s hr fcst for %s from %s' %\
