...
!
! The utility reads in the slint initialization coefficients.
!
! Ning Wang, July 2013 
!
!
!=============================================================
      ...
      program pop_read_init
#ifdef NAG
      USE F90_UNIX_ENV, ONLY : iargc
#endif
      use headers,only: testglvlheader
      USE varinfo
      ...
      USE slint, ONLY: slint_init, slint_init_read, bilinear_interp, &
                       bilinear_init_i2r, bilinear_interp_i2r, bilinear_interp_uv
...
! check and get the commandline arguments
      IF (iargc() .NE. 0) THEN
        WRITE(0,*) 'Usage: pop'
        WRITE(0,*) 'Note: Make sure that namelist file pop.nl '// &
...
!JR grid_id=228 => mx=144, my=73
      ...
      IF (is == 0) THEN
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata 0 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata_n 0 failed'
          call flush(6)
          stop
        endif
...
!JR The following is a HACK to get pop to work when number of pressure levels exceeds number of model levels
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata 1 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata2 1 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz 1 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz2 1 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of src_data 1 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of src_data2 1 failed'
          call flush(6)
          stop
        endif
      ...
      ELSE IF (is == 2) THEN
        ...
        ALLOCATE(vardata(nip * (nverlvs+1)),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata 2 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz_var 2 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz_pr 2 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz 2 failed'
          call flush(6)
          stop
        endif
        ...
        ALLOCATE(src_data(nip), tgt_data(mx*my),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of src_data 2 failed'
          call flush(6)
          stop
        endif
      ...
      ELSE IF (is == 3) THEN
        ...
        ALLOCATE(vardata(nip * (nverlvs+1)),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of vardata 3 failed'
          call flush(6)
          stop
        endif
        ...
        ALLOCATE(data_xyz_var(mx,my,nverlvs+1),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz_var 3 failed'
          call flush(6)
          stop
        endif
        ...
        ALLOCATE(data_xyz_pr(mx,my,nverlvs+1),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz_pr 3 failed'
          call flush(6)
          stop
        endif
        ...
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of data_xyz 3 failed'
          call flush(6)
          stop
        endif
        ...
        ALLOCATE(src_data(nip), tgt_data(mx*my),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of src_data 3 failed'
          call flush(6)
          stop
        endif
...
! if interpolation scheme is not 0, init the horizontal interpolation.
      IF (is /= 0) THEN
        IF (FixedGridOrder) THEN
          ...
          if (ierr.ne.0) then
            write (*,'(a)') 'pop_read_init: allocation of ll_src 0 failed'
            call flush(6)
            stop
          endif
          ...
          if (ierr.ne.0) then
            write (*,'(a)') 'pop_read_init: allocation of ll_tgt failed'
            call flush(6)
            stop
          endif
      ...
      IF (latlonfld) THEN
        ...
        ALLOCATE(ll_src(nip, 2),stat=ierr)
        if (ierr.ne.0) then
          write (*,'(a)') 'pop_read_init: allocation of ll_src 1 failed'
          call flush(6)
          stop
        endif
...
!JR Don't know the actual dimension of vardata
      ...
      INTEGER time, lunout, is2Dvar, ioerr, iret, iret_2dvar
...
! if these variables are in seperate files.
      IF (is2Dvar(var_name) == 1) THEN
        CALL read_2Dvar(nt, post_datadir, var_name, nip, nverlvs, vardata,ArchvTimeUnit, nvlp,iret_2dvar)
        iret = iret_2dvar
        RETURN
      ENDIF
...
SUBROUTINE read_2Dvar(nt, post_datadir, var_name, nip, nverlvs, vardata,ArchvTimeUnit, nvlp, iret)
      ...
      INTEGER  nt, nip, nverlvs, iret, ioerr
