...
!JR FIM default is "grib", grid_id=228 => mx=144, my=73
  ...
  subroutine post_init_slint (retcode)
    ...
    integer :: ierr                         ! status variable for allocate stmts
    ...
    if (ierr.ne.0) then
      write (*,'(a)') 'post_init_slint: allocation of data_xyz failed'
      call flush(6)
      stop
    endif
    ...
    allocate (llpoints(nip, 2),stat=ierr)
    if (ierr.ne.0) then
      write (*,'(a)') 'post_init_slint: allocation of llpoints failed'
      call flush(6)
      stop
    endif
...
! post_finalize_file closes the open GRIB file
  ...
  use fimnamelist, only: output_fmt
...
! post_init_file opens the output GRIB or (for future) netCDF file
  ...
  use fimnamelist, only: yyyymmddhhmm, max_pathlen, outputdir, output_fmt
...
! post_write_field writes a single field to the already-open GRIB file
  ...
  use fimnamelist,    only: max_varnamelen, mx, my, nsmooth_var, output_fmt, &
                            var_list, outputdir
...
!     allocating a full 3-d temporary array here
    ...
    allocate (vardata_scaled(1:nip*nlevels),stat=ierr)
    if (ierr.ne.0) then
      write (*,'(a)') 'post_write_field: allocation of vardata_scaled failed'
      call flush(6)
      stop
    endif
