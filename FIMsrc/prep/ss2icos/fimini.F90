...
subroutine fimini(nlyr,ter_in,psf_in,gz_in,p_in,t_in,q_in,		&
  ...
  use module_control  ,only: nip,nvlp1,kbl,ntra,ntrb,npp
  use fimnamelist     ,only: nvl,glvl,PrintDiags,PrintIpnDiag,pure_sig
  use module_constants,only: p1000,rd,cp,qvmin,deg_lat,perm
  use findmaxmin2
  use findmaxmin3
...
!sms$distribute(dh,2) begin
  ...
  real,intent(IN ) :: q_in (nlyr  ,nip)	   ! layer moisture
...
!sms$distribute end
  ...
  integer :: ipn
  integer :: k,k1,k2
  integer :: unitno          ! unit number to read from
  ...
  character(len=14), parameter :: fn = 'theta_coor.txt'

#include <gptl.inc>

  ! Default initialization for B tracers

  do k=ntra+1,ntra+ntrb
    tr3d(:,:,k)=0.
  end do
...
!<><><><><><><><><><><><><><>><><><><><><><><><><><><><><><><><><><><><><>
  ...
  open(unitno, file=fn, form="formatted", action="read", status='old', iostat=ios)
  if (ios /= 0) then
    write(6,*)'fimini: cannot open file=',fn,' on unit=', unitno, '. Stopping'
    stop
  end if

  read (unitno,*,iostat=ios) theta_lyrs(1:nvl)
  if (ios /= 0) then
    write(6,*)'fimini: error reading file=',fn,' on unit=', unitno, '. Stopping'
    stop
  end if
...
! --- diagnostic output
  if (PrintDiags) then
!sms$serial (<ter_in,psf_in,gz_in,p_in,t_in,q_in,qc_in,u_in,v_in,o3_in,in>:default=ignore) begin    
...
!$OMP PARALLEL DO PRIVATE (vrbos,k)
...
! --- infer virt.pot.temperature from geopotential and Exner fct
    ...
    if (vrbos) then 
      print '(a,i8/7x,a)','fimini  input at ipn =',perm(ipn),		&
...
!$OMP PARALLEL DO PRIVATE (k)
    do ipn=1,nip				! horizontal loop
      do k=1,nvl
        us3d(k,ipn)   = uswrk(k,ipn)
        vs3d(k,ipn)   = vswrk(k,ipn)
        tr3d(k,ipn,1) = thwrk(k,ipn)			! virt.pot.temperature
        tr3d(k,ipn,2) = max(qvwrk(k,ipn),qvmin)		! water vapor
        tr3d(k,ipn,3) = max(qcwrk(k,ipn),0.   )		! liquid water/condensate
        tr3d(k,ipn,4) = o3wrk(k,ipn)			! ozone
      end do
      do k=1,nvlp1
        ex3d(k,ipn)   = exwrk(k,ipn)
      end do
...
! --- choose initial pressure as tracer 1
        ...
        if (ntra > 4) then
          do k=1,nvl
            tr3d(k,ipn,5) = tr3d(k,ipn,ntra+1)	! duplicate in class A
          end do
        end if
      end if
...
! --- choose initial latitude (+90 for pos.def.) as tracer 2
        ...
        if (ntra > 5) then
          do k=1,nvl
            tr3d(k,ipn,6) = tr3d(k,ipn,ntra+2)	! duplicate in class A
          end do
        end if
      end if
    end do
...
! --- solve hydrostatic eqn for geo- and montgomery potential
    ...
    if (vrbos) then
      print 99,perm(ipn),'o u t p u t     p r o f i l e :'
...
! --- show max/min of all tracer fields in a single mid-range layer
  ...
  if(PrintDiags) then
!sms$serial (<us3d,vs3d,dp3d,ph3d,pr3d,ex3d,mp3d,tr3d,ter_in,in>:default=ignore) begin    
