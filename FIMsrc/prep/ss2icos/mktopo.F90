...
subroutine mktopo(z0in,nip)
  ...
  use fimnamelist,only: glvl,topodatfile,topoglvldir,toponpass,toposmoothfact
  ...
  integer nm,irc,iunittopo,iunitglvl,iskip,ierr
...
!
  ...
  open(iunitglvl,file=gpath,form='unformatted',status='old',iostat=ierr)
  if (ierr.ne.0) then
    write (*,'(a)') 'error in open of glvl.dat'
    stop
  endif
...
! 
  ...
  open(iunittopo,file=tpath,form='unformatted',status='old',iostat=ierr)
  if (ierr.ne.0) then
    write (*,'(a)') 'error in open of topo dat file'
    stop
  endif
