   subroutine lin2stp(xold,yold,kold,xnew,ynew,knew,vrbos,glbl)
!
...
!
   ...
   integer,intent(IN) :: glbl		!  current location in horiz.grid
...
!
   if (vrbos)								&
    write (*,101) glbl,'  lin2stp -- old profile:     x           y',	&
...
! --- column integrals (colin/clout) are computed for diagnostic purposes only
   if (xold(1).ne.xnew(1))						&
    print *,glbl,'  lin2stp warning - bottom xold and xnew differ',	&
     xold(1),xnew(1)
   if (xold(kold).ne.xnew(knew+1))					&
    print *,glbl,'  lin2stp warning - top xold and xnew differ',	&
...
!
   ...
   if (xhi.gt.xlo) then
    yinteg=0.
 2  if (xlo.ge.xxold(ko+1)) go to 1
...
!     ' xlo,xhi,xa,xb,ya,yb:',xlo,xhi,xa,xb,ya,yb
    ...
    if (xhi.le.xxold(ko+1)) go to 5
 1  ko=ko+1
    if (ko.lt.kold) go to 2
 5  yynew(k)=yinteg/(xhi-xlo)
...
!
   if (abs(clout-colin).gt.acurcy*colmx*xold(kold/2))			&
    write (*,100) glbl,' lin2stp - column intgl.error',			&
      colin,clout,(clout-colin)/colin
 100  format (i8,a,2es14.6,es9.1)
!
   if (vrbos)								&
    write (*,101) glbl,'  lin2stp -- new profile:     x           y',	&
