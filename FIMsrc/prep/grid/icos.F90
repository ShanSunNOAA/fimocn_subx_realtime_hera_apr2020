!------------------------------------------------------------------------------
!  This program generates an icosahedral grid on the sphere.   
!
!  Ning Wang, Aug 2006, implemented original version. 
!      It is partially adpated from Yuanfu Xie's icos.F90. The main feature 
!      of this new version is that the grid points created are arranged in a 
!      default order (ij order within each rhombus) on the globe, which 
!      provides the base for MPP to effectively partition the icosahedral 
!      grid for parallel model run.  Based on the default order, the brute-
!      force proximity computation (an O(n^2) operation) is replaced with a 
!      much more efficient O(n) time pattern-based computation, which allows 
!      higher resolution icosahedral grid (at grid level 7 and above) to be 
!      computed dynamically. 
!   
!  Revision history:
!  Ning Wang, May 2007, Two more new grid generation algorithms are developed
!      to enhanced the geometric properties of the icosahedral grid. They are 
!      called modified recursive and non-recursive method.
!
!  Ning Wang, Aug 2008, added several subroutines to accommodate newly 
!      developed triangle_trisect subroutines. The revision includes adding 
!      an array to specify bisect or tri-sect at each refinment 
!      level, and dynamically compute 'nip' from the subdivision specifi-
!      cation array.
!------------------------------------------------------------------------------
   ...
   PROGRAM Icosahedral_grid
     ...
     USE DataStru 
     USE fimnamelist,only: readnl_serial, glvl, gtype, SubdivNum
     ...
     REAL(KIND=8) latlon_d(2), theta, lambda 
...
!     SubdivNum = 0
     ...
     SELECT CASE (gtype)
       CASE (0)
         PRINT*, 'Grid construction algorithm: Standard recursive.'
       CASE (2)
         PRINT*, 'Grid construction algorithm: Modified recursive.'
       CASE (3)
         PRINT*, 'Grid construction algorithm: Modified nonrecursive.'
       CASE DEFAULT
         PRINT*, 'Incorrect specification of grid construction algorithm'
      END SELECT
      PRINT*, 'Grid resolution level:', glvl
...
! compute icos grid starting from the top grid
     CALL comp_grid(top_triangle, SubdivNum, gtype)
...
SUBROUTINE comp_grid(top_triangle, SubdivNum, gtype)
     USE DataStru

     IMPLICIT NONE
 
     TYPE(Triangle) :: top_triangle(20)
     INTEGER gtype, SubdivNum(20)
...
! For each of the 20 triangles, recursively bisect them, and
     DO i = 1,20
       ...
       IF (gtype == 0) THEN
         CALL bisect_triangle_nasr(top_triangle(i)%vertex(1), &
           top_triangle(i)%vertex(2), top_triangle(i)%vertex(3), 0, SubdivNum)
       ELSE IF (gtype == 2) THEN
         CALL bisect_triangle_nasr_mrc(top_triangle(i)%vertex(1), &
           top_triangle(i)%vertex(2), top_triangle(i)%vertex(3), &
           ph, ph, ph, 0, 0, SubdivNum)
!       ELSE IF (type == 3) THEN
!         CALL msect_triangle(top_triangle(i)%vertex(1), &
!           top_triangle(i)%vertex(2), top_triangle(i)%vertex(3))
       ELSE IF (gtype == 10) THEN
         IF (SubdivNum(1) == 2) THEN
           CALL bisect_triangle_nasr(top_triangle(i)%vertex(1), &
             top_triangle(i)%vertex(2), top_triangle(i)%vertex(3), 0, SubdivNum)
         ENDIF
         IF (SubdivNum(1) == 3) THEN
           CALL trisect_triangle_nasr(top_triangle(i)%vertex(1), &
             top_triangle(i)%vertex(2), top_triangle(i)%vertex(3), 0, SubdivNum)
         ENDIF
       ENDIF
     END DO
