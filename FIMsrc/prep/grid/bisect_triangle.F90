...
!  Oct 2007:  Added a new scheme for recursive refinement. N. Wang
!  Jun 2009:  Added new bisect_triangle and bisect_r_triangle
!             subroutines to allow combined bisection and 
!             tri-sectiuon refinements. N. Wang 
!  Feb 2012:  Added a new pair of subroutines to allow combined
!             bisection and tri-section refinements to work with
!             the modified recursive construction. N. Wang 
!====================================================================
...
RECURSIVE SUBROUTINE bisect_triangle_nasr_mrc(a, b, c, a1, a2, a3, level, side, msec)
...
! check to see if recursive call bottoms out.
     ...
     CALL compute_index(a%seqnum, b%seqnum, c%seqnum, a_b%seqnum, a_c%seqnum, b_c%seqnum, clevel, 1) 
!  PRINT*, a%seqnum,b%seqnum,c%seqnum,a_b%seqnum,a_c%seqnum,b_c%seqnum
...
! recursive calls to create next level grid
    IF (msec(clevel + 1) == 2) THEN
      CALL bisect_triangle_nasr_mrc(a, a_b, a_c, b_b_c, b_c_c, anc1, clevel, 3, msec)
      CALL bisect_triangle_nasr_mrc(a_b, b, b_c, a_a_c, a_c_c, anc2, clevel, 2, msec)
      CALL bisect_triangle_nasr_mrc(a_c, b_c, c, a_a_b, a_b_b, anc3, clevel, 1, msec)
      CALL bisect_r_triangle_nasr_mrc(a_b, a_c, b_c, anc1, anc2, anc3, clevel, 4, msec)
    ENDIF
    IF (msec(clevel + 1) == 3) THEN
      CALL trisect_triangle_nasr(a, a_b, a_c, clevel, msec)
      CALL trisect_triangle_nasr(a_b, b, b_c, clevel, msec)
...
END SUBROUTINE bisect_triangle_nasr_mrc

RECURSIVE SUBROUTINE bisect_r_triangle_nasr_mrc(a, b, c, a1, a2, a3, level, side, msec)
...
! recursive calls to create next level grid
     IF (msec(clevel + 1) == 2) THEN
       CALL bisect_r_triangle_nasr_mrc(a, a_b, a_c, b_b_c, b_c_c, anc1, clevel, 3, msec)
       CALL bisect_r_triangle_nasr_mrc(a_b, b, b_c, a_a_c, a_c_c, anc2, clevel, 2, msec)
       CALL bisect_r_triangle_nasr_mrc(a_c, b_c, c, a_a_b, a_b_b, anc3, clevel, 1, msec)
       CALL bisect_triangle_nasr_mrc(a_b, a_c, b_c, anc1, anc2, anc3, clevel, 4, msec)
     ENDIF
     IF (msec(clevel + 1) == 3) THEN
       CALL trisect_r_triangle_nasr(a, a_b, a_c, clevel, msec)
       CALL trisect_triangle_nasr(a_b, a_c, b_c, clevel, msec)
...
END SUBROUTINE bisect_r_triangle_nasr_mrc
