...
!  integration of the (finite volume) model.  
!
...
!==========================================================
      ...
      use headers       ,only: writecurveheader,writeglvlheader
      use fimnamelist  ,only: readnl_serial,nprocs,curve,glvl,NumCacheBlocksPerPE
!      use module_control,only: control
      IMPLICIT NONE
      ...
      call readnl_serial ! To read the namelist
 !     call control
...
!       END DO
       ...
       ! Voronoi corners:
       DO j=1,jmax
         ! skip the 6th neighbor for a 5-neighbor point, additional safeguard
         IF (nb(j) .GT. 0) THEN
           k = j+1
           IF (k .GT. jmax) k = k-jmax
