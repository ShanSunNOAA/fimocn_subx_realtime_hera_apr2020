...
!*********************************************************************
  ...
  subroutine mxlyr1(nstep,leapn,theta,dens,dp,pres,temp,saln,		&
  ...
  use hycom_constants ,only: wet,batrop,spcifh,thref,epsil, 		&
                             sigjmp,salmin,dpthmn,onem,onecm,onemm,	&
...
!SMS$DISTRIBUTE END
  ...
  character :: string*16
  real,parameter :: ea1=0.6 , ea2=0.3,			& ! Gaspar coefficients
                    em1=0.45, em2=2.6, em3=1.9, em4=2.3, em5=0.6
...
! --- determine turb.kin.energy generation due to wind stirring (ustar3)
! --- and diabatic forcing (buoyfl).

! --- g a s p a r   mixed-layer t.k.e. closure

   dpth=thkold/onem
   ekminv=abs(corio(i))/max(epsil,ustar(i))
   ...
   ape=cp3*ustar3-cp1*dpth*buoyfl				! m^3/s^3

   if(ape.lt.0.) then						! detrainment
    turgen=(grvity*delt/thref**3)*ape				! kg^3/(s^4 m^5)

   else								! entrainment
    cc4=2.*em4/(em1*em1) * alf1*alf1
    spe=(em2+em3)*ustar3-0.5*dpth*buoyfl
    turgen=(grvity*delt/thref**3)*(sqrt((.5*ape-cp1*spe)**2		&
          +2.*cc4*ape*spe)-(.5*ape+cp1*spe))/(cc4-cp1)		! kg^3/(s^4 m^5)
   end if

   if (vrbos) then
    print '(i9,i7,a/16x,6es10.2)',nstep,perm(i),			&
      '   surflx    pmevap     ustar    buoyfl    turgen',		&
       surflx(i),pmevap(i),ustar(i),buoyfl,turgen
   end if
...
!        ' p(k),p(k+1),pnew=',prcol(k)/onem,prcol(k+1)/onem,	&
!        pnew/onem
!      end if
...
! --- (mixed layer  r e c e d e s)
    ...
    thknew=min(thkold,grvity*cp3/(cp1*max(epsil,obuinv)))

! --- don't allow mixed layer to get too shallow
    thknew=max(dpthmn*onem,thknew)

    if (vrbos) then
     print '(i9,i7,a,es11.3,a)',nstep,perm(i),' detrain',		&
       (thkold-thknew)/onem,' m of water'
    end if
...
! --- having a density close to that of layer -kdetr-. detrain lower sublayer.
   ...
   if (kdetr.gt.kdm) then
    print '(i9,i7,a,i4)',nstep,perm(i),					&
      ' (mxlyr1) fossil layer denser than bottom layer'
   end if
...
! --- this is to avoid generating unrealistic T,S combinations during unmixing
    ...
    if (vrbos) then
     ...
     if (slowr.le.smax) then
      ...
      print '(a,i7/a,8f8.3)','(mxlyr1) unmix error at ipn=',perm(i),	&
...
!   call findmxmn2(dpchg,kdm,nip,k,'(mxlayr) dpchg'//string,wet)
  ...
  return
  end subroutine mxlyr1
end module hycom_mxlyr1
