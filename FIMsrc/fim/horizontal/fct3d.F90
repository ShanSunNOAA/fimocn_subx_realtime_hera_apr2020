...
! --- thko,thkn - layer thickness at previous and new time step
  ...
  use module_control  ,only: npp,nip,ntra,ntrb
...
!SMS$PARALLEL END
  ...
  if (TimingBarriers) then
    ret = gptlstart ('fct3d_barrier')
!SMS$BARRIER
    ret = gptlstop ('fct3d_barrier')
  end if
  ret = gptlstart ('fct3d_exchange')
!SMS$EXCHANGE(fields,thko,thkn)
  ret = gptlstop ('fct3d_exchange')
