...
! -+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
   ...
   use fimnamelist,     only: PrintIpnDiag, TimingBarriers,	&
                              stencl_frst,stencl_last,stencl_step	
   use module_constants,only: deg_lat, deg_lon, nprox, prox, pi, perm

   implicit none
   real     ,intent(IN) :: sclfac	! scale factor (< 0 for E-format print)
...
!  integer,parameter :: num_rings=2	! number of concentric rings
   ...
   character map(idm,jdm)*1,string*8
   ...
   integer iloc    (maxpts),jloc    (maxpts),				&
           cell    (maxpts),cellglb (maxpts),				&
           cumucell(maxpts),cumuglb (maxpts),				&
           cell1   (maxpts),cell1glb(maxpts)
...
!SMS$PARALLEL (dh,ipn) BEGIN
      ...
      if (TimingBarriers) then
        ret = gptlstart ('stencilprint_barrier')
!SMS$BARRIER
        ret = gptlstop ('stencilprint_barrier')
      end if
      ret = gptlstart ('stencilprint_exchange')
!SMS$EXCHANGE(field)
      ret = gptlstop ('stencilprint_exchange')
...
!    print '(a,i4,a/(2i9,2f9.2,f9.1))','considering',ntot,' cells', 	&
...
!  print '(a/(i4,2i9,2f9.2,f9.1))',					&
!   'all cluster points have now been identified:',			&
!    (n,cell(n),cellglb(n),alat(n),alon(n),valu(kfrst,n),n=1,ntot)
...
!  print 101,cell(1),cellglb(1),alat(1),alon(1),0.,0.,x(1),y(1)
101 format (2i8,2f8.2,f8.3,f8.2,2f8.3)
   do n=2,ntot
...
!   map(i+1:i+7,j)=string(1:7)		! doesn't work
    do m=1,7
   ...
   do k=kfrst,klast,kstep
    do n=1,ntot
     ...
     if (sclfac.gt.0.) then
      ...
      write (string,'(es8.1)') valu(k,n)
      string(5:7)=string(6:8)		! omit the 'E'
     end if
!    map(i+1:i+7,j)=string(1:7)		! doesn't work
     do m=1,7
      map(i+m,j)=string(m:m)
