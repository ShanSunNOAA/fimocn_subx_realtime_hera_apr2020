...
!
      CALL CUP_ct(ktop,tracer,j,AAEQ,T,Q,TER11,PRET,P,tracert, &
           hstary,DT,PSUR,US,VS,trdep,tcrit,                         &
           xlv,r_v,cp,g,0,0,npr,num_chem,chemopt,          &
           numgas,itf,jtf,ktf,                  &
           its,ite, jts,jte, kts,kte                           )
 ...
 100    continue
   ...
   SUBROUTINE CUP_ct(ktop,tracer,J,AAEQ,T,Q,Z1,                        &
              PRE,P,tracert,hstary,DTIME,PSUR,US,VS,TRFALL,TCRIT,             &
...
!
      ...
      call cup_dd_moisture_3d(zd,hcd,hes_cup,qcd,qes_cup, &
           ...
           pwev,bu,qrcd,q,he,t_cup,2,xl,0, &
           itf,jtf,ktf,&
           its,ite, jts,jte, kts,kte)
!
...
!
       ...
       call cup_up_tracer(ierr,tcrit,t,pre,z_cup,p,tracer,tre_cup,tr_up,tr_pw, &
                   ...
                   num_chem,numgas,chemopt,itf,jtf,ktf,&
      ...
      if(chem_opt == 500)then
         do nv=1,num_chem
            do i=its,ite
               IF(ierr(I).eq.0)then
                  do k=kts,ktop(i)
                    trfall(i,nv)=trfall(i,nv)+tr_pw(i,k,nv)*xmb(i)*dtime
                  enddo
               endif
            enddo
         enddo
...
!
     ...
     integer                              ::                           &
        iall,i,k,iwd,nv
     real                                 ::                           &
        alpha,trcc,trch,dh,qrch,c0,dz,radius,airm,dens
     integer                              ::                           &
       iaer,igas
!
...
!              tr_up(i,k,nv)=trcc
        if(chemopt.ge.300.and. chemopt .lt. 500 )then
               if(nv.gt.numgas)then
                 ...
                 tr_c(i,k,nv)=alpha*tr_up(i,k,nv)
                 trch=tr_up(i,k,nv)-tr_c(i,k,nv)
!
        ...
        else if(chemopt.ge.500)then
               alpha = 0.    ! scavenging factor
               if(chemopt == 501 .or. chemopt == 502)alpha = .5    ! scavenging factor
               tr_c(i,k,nv)=alpha*tr_up(i,k,nv)
               trch=tr_up(i,k,nv)-tr_c(i,k,nv)
               trcc=(tr_up(i,k,nv)-trch)/(1.+c0*dz*zu(i,k))
               tr_pw(i,k,nv)=c0*dz*trcc*zu(i,k)
...
!       else if(chemopt.ge.300.and.nv.gt.numgas)then
!              alpha = .5    ! scavenging factor
!              if(nv.eq.p_bc1 .or. nv.eq.p_oc1 .or. nv.eq.p_dms)alpha=0.
!              if(nv.eq.p_sulf .or. nv.eq.p_seas_1 .or. nv.eq.p_seas_2)alpha=1.
