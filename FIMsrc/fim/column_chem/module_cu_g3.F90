...
! LOCAL VARS
   ...
   REAL, DIMENSION( its:ite , jts:jte ) ::            MASSI_FLX,    &
     ...
     real,    dimension (its:ite,kts:kte) ::                    &
        SUBT,SUBQ,OUTT,OUTQ,OUTQC,phh,subm,cupclw,outu,outv
...
!
      CALL CUP_enss_3d(outqc,j,AAEQ,T2d,Q2d,TER11,subm,TN,QO,PO,PRET,    &
           ...
           outu,outv,maxiens,maxens,maxens2,maxens3,ensdim,              &
           APRi_GR,APRi_W,APRi_MC,APRi_ST,APRi_AS,                       &
...
!                raincv(i,j)=pret(i)*dt
            ...
            ENDDO
            if(imomentum.eq.1.and.high_resolution.eq.0)then
            IF(PRESENT(RUCUTEN)) THEN
                DO K=kts,ktf
                DO I=ibegc,iendc
                   RUCUTEN(i,k,j)=outu(i,k)*cuten(i)
                ENDDO
                ENDDO
            ENDIF
            IF(PRESENT(RUCUTEN)) THEN
                DO K=kts,ktf
                DO I=ibegc,iendc
                   RVCUTEN(i,k,j)=outv(i,k)*cuten(i)
                ENDDO
                ENDDO
            ENDIF
            ELSE IF (imomentum.eq.1.and.high_resolution.eq.1)then
!             CALL wrf_error_fatal('Do not run this option with high resolution option')
              stop
            ENDIF
...
!......     QSTEN STORES GRAUPEL TENDENCY IF IT EXISTS, OTHERISE SNOW (V2)     
 ...
 100    continue
   ...
   SUBROUTINE CUP_enss_3d(OUTQC,J,AAEQ,T,Q,Z1,sub_mas,                 &
              ...
              omeg,direction,massflx,outu,outv,maxiens,                &
  ...
  ! pre    = output precip
     real,    dimension (its:ite,kts:kte)                              &
        ,intent (inout  )                   ::                           &
        OUTT,OUTQ,OUTQC,subt,subq,sub_mas,cupclw,outu,outv
     real,    dimension (its:ite)                                      &
        ,intent (out  )                   ::                           &
        pre
!+lxz
...
!
! following for momentum transport if needed
!
     real,    dimension (its:ite,kts:kte) ::           &
        uc,vc,ucd,vcd,u_cup,v_cup,dellau,dellav,ukb,vkb
...
!
      call cup_env_clev(t,qes,q,he,hes,z,p,qes_cup,q_cup,he_cup, &
           ...
           its,ite, jts,jte, kts,kte)
      if(imomentum.eq.1)then
      call cup_env_clev_uv(us,vs,u_cup,v_cup,ierr,          &
           its,ite, jts,jte, kts,kte, itf,ktf)
      endif
...
!
      ...
      call cup_dd_he(heso_cup,zdo,hcdo,zo_cup,cdd,mentrd_rate, &
           ...
           itf,jtf,ktf, &
           its,ite, jts,jte, kts,kte)
! for momenum we only need to do this once!!!
      if(imomentum.eq.1)then
      call cup_up_uv(k22,ukb,vkb,zo_cup,cd,cdd,mentr_rate,      &
              mentrd_rate,vcd,ucd,jmin,u_cup,              &
              v_cup,uc,vc,kbcon,ierr,us,vs,                    &
              its,ite, jts,jte, kts,kte, itf,ktf    )
       endif
!
...
!
      ...
      call cup_dellabot(ipr,jpr,qo_cup,ierr,zo_cup,po,qrcdo,edto, &
           ...
           its,ite, jts,jte, kts,kte)
      if(imomentum.eq.1)then
      call cup_dellabotuv(ipr,jpr,u_cup,v_cup,ierr,zo_cup,po,ucd,vcd,edto, &
           zdo,cdd,us,vs,dellau,dellav,j,mentrd_rate,zo,g,&
           its,ite, jts,jte, kts,kte, itf,ktf)
      endif
...
!
      call cup_dellas_3d(ierr,zo_cup,po_cup,hcdo,edto,zdo,cdd,    &
           ...
           its,ite, jts,jte, kts,kte)
      if(imomentum.eq.1)then
      call cup_dellasuv(ierr,zo_cup,po_cup,ucd,vcd,edto,zdo,cdd,        &
              us,vs,dellau,dellav,j,mentrd_rate,zuo,g,                    &
              cd,uc,vc,ktop,k22,kbcon,mentr_rate,jmin,u_cup,           &
              v_cup,kdet,k22,ipr,jpr,'deep',                            &
              its,ite, jts,jte, kts,kte, itf,ktf              )
      endif
!
...
!
      ...
      do i=its,itf
           ...
           if(pre(i).gt.0.)then
           do k=kts,ktf-1
             outu(i,k)=dellau(i,k)*xmb(i)
             outv(i,k)=dellav(i,k)*xmb(i)
           enddo
           endif
      enddo
!
...
!
   ...
   INTEGER                        :: ifs,ife,jfs,jfe,its,ite,jts,jte,ido,jdo,cugd_spread
   LOGICAL                        :: new
!
! Flags relating to the optional tendency arrays declared above
! Models that carry the optional tendencies will provdide the
...
! SET START AND END POINTS FOR TILES
      !$OMP PARALLEL DO   &
      !$OMP PRIVATE ( ij ,ifs,ife,jfs,jfe,its,ite,jts,jte, i,j,k,ikk1,ikk2,ikk11,qmemf,qmem1,qmem2,qmem)

      DO ij = 1 , num_tiles
        its = i_start(ij)
        ite = min(i_end(ij),ide-1)
        jts = j_start(ij)
        jte = min(j_end(ij),jde-1)
...
!-------------------------------------------------------
   SUBROUTINE cup_up_uv(k22,ukb,vkb,z_cup,cd,cdd,entr,       &
              entrd,vcd,ucd,jmin,u_cup,v_cup,uc,vc,kbcon, &
              ierr,u,v,                                      &
              its,ite, jts,jte, kts,kte, itf,ktf    )
...
!
     ...
     real                                                 ::           &
      ...
      ukb(i)=u_cup(i,k22(i))
      vkb(i)=v_cup(i,k22(i))
      ukbd(i)=u_cup(i,jmin(i))
      vkbd(i)=v_cup(i,jmin(i))
      do k=kts,ktf
        ucd(i,k)=u_cup(i,k)               
        vcd(i,k)=v_cup(i,k)
      enddo
      ...
      do i=its,itf
        if(k.gt.kbcon(i).and.ierr(i).eq.0.)then
           DZ=Z_cup(i,K)-Z_cup(i,K-1)
           UC(i,K)=(UC(i,K-1)*(1.-.5*CD(i,K)*DZ)+entr* &
                DZ*U(i,K-1))/(1.+entr*DZ-.5*cd(i,k)*dz)
           VC(i,K)=(VC(i,K-1)*(1.-.5*CD(i,K)*DZ)+entr* &
                DZ*V(i,K-1))/(1.+entr*DZ-.5*cd(i,k)*dz)
        endif
      enddo
      enddo
      do i=its,itf
...
!     
      do ki=jmin(i)-1,1,-1
         DZ=Z_cup(i,Ki+1)-Z_cup(i,Ki)
         uCD(i,Ki)=(uCD(i,Ki+1)*(1.-.5*CDD(i,Ki)*DZ) &
                  +entr*DZ*u(i,Ki) &
                  )/(1.+entr*DZ-.5*CDD(i,Ki)*DZ)
         vCD(i,Ki)=(vCD(i,Ki+1)*(1.-.5*CDD(i,Ki)*DZ) &
                  +entr*DZ*v(i,Ki) &
                  )/(1.+entr*DZ-.5*CDD(i,Ki)*DZ)
      enddo
!     
...
!--- end loop over i
   ...
   SUBROUTINE cup_env_clev_uv(us,vs,u_cup,v_cup,ierr,          &       
           its,ite, jts,jte, kts,kte, itf,ktf)
   IMPLICIT NONE
...
!
   ...
   SUBROUTINE cup_dellabotuv(ipr,jpr,u_cup,v_cup,ierr,z_cup,p_cup,  &
              ucd,vcd,edt,zd,cdd,u,v,dellau,dellav,j,mentrd_rate,z,g,     &
              its,ite, jts,jte, kts,kte, itf,ktf    )
  ...
  !
     ...
     real,    dimension (its:ite,kts:kte)              &
        ,intent (in  )                   ::                           &
        z_cup,p_cup,ucd,vcd,zd,cdd,u,v,z,u_cup,v_cup
     real,    dimension (its:ite)                      &
        ,intent (in   )                   ::                           &
        edt
     real                                              &
...
!
      ...
      subin=-EDT(I)*zd(i,2)
      detdo=detdo1+detdo2-entdo+subin
      DELLAU(I,1)=(detdo1*.5*(UCD(i,1)+UCD(i,2)) &
                 +detdo2*ucd(i,1) &
                 +subin*u_cup(i,2) &
                 -entdo*u(i,1))*g/dp
      DELLAV(I,1)=(detdo1*.5*(VCD(i,1)+VCD(i,2)) &
                 +detdo2*vcd(i,1) &
                 +subin*v_cup(i,2) &
                 -entdo*v(i,1))*g/dp
 100  CONTINUE

   END SUBROUTINE cup_dellabotuv
   SUBROUTINE cup_dellasuv(ierr,z_cup,p_cup,ucd,vcd,edt,zd,cdd,        &
              u,v,dellau,dellav,j,mentrd_rate,zu,g,                    &
              cd,uc,vc,ktop,k22,kbcon,mentr_rate,jmin,u_cup,           &
              v_cup,kdet,kpbl,ipr,jpr,name,                            &
              its,ite, jts,jte, kts,kte, itf,ktf              )
  ...
  !
     ...
     real,    dimension (its:ite,kts:kte)              &
        ,intent (in  )                   ::                           &
        z_cup,p_cup,ucd,vcd,zd,cdd,u,v,uc,vc,cd,zu,u_cup,v_cup
     real,    dimension (its:ite)                      &
        ,intent (in   )                   ::                           &
        edt
     real                                              &
...
!        
         ...
         dellav(i,k)=(subin*v_cup(i,k+1) &
                    -subdown*v_cup(i,k) &
                    +detup*.5*(vC(i,K+1)+vC(i,K)) &
         ...
         dellau(i,k)=(subin*u_cup(i,k+1) &
                    -subdown*u_cup(i,k) &
                    +detup*.5*(uC(i,K+1)+uC(i,K)) &
