...
!            print*,'before calling set_volcaer lavoflg'
      if ( lavoflg ) then
!            print*,'calling set_volcaer'
...
!            print*,' in set_volcaer'
      if ( .not. allocated(ivolae) ) then
!            print*,' in set_volcaer allocating ivolvae'
...
!           print*,'calling trop update',lalwflg,laswflg
        call trop_update
!           print*,'after calling trop update'
      endif

      if ( lavoflg ) then              ! update yearly stratospheric volcanic aerosol data
!           print*,'before calling volc update'
        call volc_update
!           print*,'after calling volc update'
      endif
...
!JR These things need to be thread-local for OMP to work
      logical :: laddsw, laersw, laddlw, laerlw
...
!JR Set initial values of these 4 things
      laddsw = .false.
      laersw = .false.
      laddlw = .false.
      laerlw = .false.
...
!  ---  conversion constants
!jbao orig      real (kind=kind_phys), parameter :: dltg = 360.0 / float(imxae)
      real (kind=kind_phys), parameter :: dltg = 360.0 / 72.
