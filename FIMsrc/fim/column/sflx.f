...
!tgs &       vegtyp, soiltyp, slopetyp, shdmin, alb, snoalb,            &
...
!jbao orig      real (kind=kind_phys), intent(out) :: shdfac, snowh, albedo,      &
      real (kind=kind_phys), intent(inout) :: shdfac, snowh, albedo,      &
...
Ctgs for MODIS IF VEGTYPE=15 (GLACIAL-ICE), RE-SET ICE FLAG = -1 C(GLACIAL-ICE)
...
!      if (vegtyp == 13) then
!tgs for MODIS 15
      IF ((alt_land. and.VEGTYP .EQ. 15) .or.
     &    (.not.alt_land .and. VEGTYP .EQ. 13)) THEN   ! GFS landuse
        ice = -1
        shdfac = 0.0
      endif
...
!            z0, czil, xlai, csoil )                                       !
       if (alt_land) then   ! for MODIS
!tgs - seasonal variation of LAI and Z0 from WRF Noah LSM
       IF ( SHDFAC .ge. SHDMAX ) THEN
          XLAI  = LAIMAX
          Z0    = Z0MAX
       ELSE IF ( SHDFAC .le. SHDMIN ) THEN
          XLAI  = LAIMIN
          Z0    = Z0MIN
       ELSE
     ...
     1             +(INTERP_FRACTION*LAIMAX)
           Z0    = ((1.0 - INTERP_FRACTION)*Z0MIN)
     1             +(INTERP_FRACTION * Z0MAX)
         ELSE
           XLAI  = 0.5 * LAIMIN    + 0.5 * LAIMAX
           Z0    = 0.5 * Z0MIN     + 0.5 * Z0MAX
         ENDIF
...
!      if (vegtyp > defined_veg) then
      IF ((.not. alt_land .and. VEGTYP .GT. DEFINED_VEG) .or.
     &          (alt_land .and. VEGTYP .GT. DEFINED_VEG_MODIS)) THEN
        WRITE(*,*) 'VEGTYP =',VEGTYP,'DEFINED_VEG',DEFINED_VEG
...
!     EMISSMAX  = EMISSMAXTBL  (VEGTYP)
      NROOT = NROOT_DATA_MODIS(VEGTYP)
      SNUP = SNUPX_MODIS(VEGTYP)
      RSMIN = RSMTBL_MODIS(VEGTYP)
      RGL = RGLTBL_MODIS(VEGTYP)
      HS = HSTBL_MODIS(VEGTYP)
      LAIMIN    = LAIMIN_DATA   (VEGTYP)
      LAIMAX    = LAIMAX_DATA   (VEGTYP)
      Z0MIN     = Z0MIN_DATA    (VEGTYP)
      Z0MAX     = Z0MAX_DATA    (VEGTYP)
!     ALBEDOMIN = ALBEDOMINTBL (VEGTYP)
!     ALBEDOMAX = ALBEDOMAXTBL (VEGTYP)
      endif

!     if (vegtyp == bare) shdfac = 0.0
      if( (.not. alt_land .and. VEGTYP .EQ. BARE) .or.
     &           (alt_land .and. VEGTYP .EQ. BARE_MODIS)) then
          SHDFAC = 0.0
      endif
...
!  ---  outputs:
!jbao orig      real (kind=kind_phys),  intent(out) :: smc(nsoil), runoff1,       &
      real (kind=kind_phys),  intent(inout) :: smc(nsoil), runoff1,       &
...
!  ---  outputs:
!jbao orig     real (kind=kind_phys), intent(out) :: sh2oout(nsoil), runoff3,    &
      real (kind=kind_phys), intent(inout) :: sh2oout(nsoil), runoff3,    &
     &       smc(nsoil)
