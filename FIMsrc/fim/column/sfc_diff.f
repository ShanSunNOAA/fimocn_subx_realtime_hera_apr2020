...
!          if (vegtype(i) == 7) then
!tgs for MODIS - 10
        if((alt_land .and. VEGTYPE(I).eq.10) .or.    ! MODIS
     &     (.not.alt_land .and.VEGTYPE(I).eq.7)) then     ! GFS landuse
! grassland
            z0max(i) = exp( ((1.-shdmax(i))**2)*log(0.01)+
...
!        GROUNDCOVER
!tgs for MODIS 7
        if((alt_land .and. VEGTYPE(I).eq.7) .or.    ! MODIS
     &     (.not.alt_land .and.VEGTYPE(I).eq.8)) then     ! GFS landuse
! open shrublands
            z0max(i) = exp( ((1.-shdmax(i))**2)*log(0.01)+
...
!          if (vegtype(i) == 9) then
!tgs - for MODIS 6
        if((alt_land .and. VEGTYPE(I).eq.6) .or.    ! MODIS
     &     (.not.alt_land .and.VEGTYPE(I).eq.9)) then     ! GFS landuse
! closed shrublands
            z0max(i) = exp( ((1.-shdmax(i))**2)*log(0.01)+
...
!tgs for MODIS  16
        if((alt_land .and. VEGTYPE(I).eq.16) .or.    ! MODIS
     &     (.not.alt_land .and.VEGTYPE(I).eq.11)) then     ! GFS landuse
