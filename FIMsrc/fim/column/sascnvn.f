...
!     real(kind=kind_phys) detad
     ...
     &                     dv1q,    dv2h,    dv2q,    dv1u,
     &                     dv1v,    dv2u,    dv2v,    dv3q,
     &                     dv3h,    dv3u,    dv3v,    dzmax, ! jbao 2014
     &                     dz,      dz1,     e1,      edtmax,
...
!
      ...
      real(kind=kind_phys) test, redfac ! jbao positive definite 
!     save pcrit, acritt
...
! jbao 2014
            dzmax   = max(dzmax, (zi(i,k+1) - zi(i,k)))
! jbao 2014
...
! jbao 2014
      do k = 1, km1
        do i=1,im
          ptem = xlamud(i) - 1/dzmax
          xlamue(i,k) = max(xlamue(i,k), ptem)
        enddo
      enddo
      do i=1,im
...
c
      do i = 1, im
        ...
        qcond(i) = 0.
      enddo
!jbao positive definite 
      val     =             1.e-10
      redfac = 1.
      do k = 1, km
        do i = 1, im
         if (cnvflg(i) .and. k.le.ktcon(i)) then
         test =  dellaq(i,k) * xmb(i) * dt2
         !if ((q1(i,k)+test).le. val) then
         !redfac=min(redfac,(val - q1(i,k))/test)
         if ((q1(i,k)+test).le. 0) then
         test=max(test,1.e-10)		! safeguard against division by zero
         redfac=min(redfac,(- q1(i,k))/test)
         endif
         endif
        enddo
      enddo
!jbao positive definite 
      do k = 1, km
        do i = 1, im
         if (cnvflg(i) .and. k .le. kmax(i)) then
            if(k.le.ktcon(i)) then
              dellaq(i,k)=redfac*dellaq(i,k)
              dellah(i,k)=redfac*dellah(i,k)
              dellau(i,k)=redfac*dellau(i,k)
              dellav(i,k)=redfac*dellav(i,k)
              dellat = (dellah(i,k) - hvap * dellaq(i,k)) / cp
...
c
      do i = 1, im
        ...
        delq2(i) = 0.
        flg(i) = cnvflg(i)
        xmb(i) = redfac*xmb(i) !jbao positive definite testing
      enddo
      do k = km, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k .le. kmax(i)) then
