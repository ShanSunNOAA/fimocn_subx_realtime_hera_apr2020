...
!  ---  output:
!jbao orig      real (kind=kind_phys), dimension(im), intent(out) :: sncovr1,     &
...
!tgs &       vtype, stype, slope, shdmin1d, alb, snoalb1d,              &
     &       vtype, stype, slope, shdmin1d, shdmax1d, alb, snoalb1d,    &
!  ---  input/outputs:
...
!   --- ...  compute qsurf (specific humidity at sfc)
      ...
      do i = 1, im
        if (flag_iter(i) .and. flag(i)) then
          tem     = 1.0 / rho(i)
          hflx2(i) = hflx(i)                    ! hflx2: sensible heatflux in W/m2
          evap2(i) = evap(i)                    ! evap2: latent heatflux in W/m2
